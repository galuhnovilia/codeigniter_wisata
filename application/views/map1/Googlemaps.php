...
class Googlemaps {
    ...
    var $apiKey = 'AIzaSyAgBiLRAb79hMx-oAZFz295qRDQONCeoLI';       // If you've got an API key you can use it by passing this parameter. Setup an API key here: https://code.google.com/apis/console
    ...
    function add_marker($params = array()) {
        ...
        if ($marker['position'] != "") {
            ...
            ';
                ...
                $this->markersInfo['marker_' . $marker_id]['longitude'] = trim($explodePosition[0]);
                $this->markersInfo['marker_' . $marker_id]['latitude'] = trim($explodePosition[1]);
                
            } else {
                $lat_long = $this->get_lat_long_from_address($marker['position']);
                $marker_output .= '
            var myLatlng = new google.maps.LatLng(' . $lat_long[0] . ', ' . $lat_long[1] . ');';
            $this->markersInfo['marker_' . $marker_id]['longitude'] = $lat_long[0];
                $this->markersInfo['marker_' . $marker_id]['latitude'] = $lat_long[1];
